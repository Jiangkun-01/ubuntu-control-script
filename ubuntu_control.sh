...
while true; do
    ...
    case \$choice in
        ...
        10)
            ...
            ;;
        11)
            echo -e "\${GREEN}SSH 远程管理:\${NC}"
            echo "1) 远程登录"
            echo "2) 复制文件到远程主机"
            read sub_choice
            case \$sub_choice in
                1) read -p "请输入远程地址（user@host）: " remote; ssh \$remote ;;
                2) read -p "请输入文件路径: " file; read -p "请输入远程地址（user@host）: " remote; scp \$file \$remote:~ ;;
            esac
            ;;
        12)
...
# 给脚本添加执行权限
chmod +x ~/ubuntu_menu.sh

# 将快捷命令添加到 ~/.bashrc 或 ~/.zshrc
echo 'alias h="bash ~/ubuntu_menu.sh"' >> ~/.bashrc

# 重新加载 ~/.bashrc 配置
source ~/.bashrc

echo "操作完成！你现在可以通过输入 'h' 来调用该脚本了。"
